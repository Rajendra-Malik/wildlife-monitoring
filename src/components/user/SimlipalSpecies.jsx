...
const SimlipalSpecies = () => {
    const [spinner, setSpinner] = useState(false);
    const [simlipalSpecies, setSimlipalSpecies] = useState([]);
    const navigate = useNavigate();


    useEffect(() => {
        async function getSimlipalSpeciesData() {
            setSpinner(true);
            try {
                const result = await axios.get("http://localhost/wildlife_backend/api/user/similipal_species.php");
                if (result.status === 200) {
                    if (result.data.status === "1") {
                        setSimlipalSpecies(result.data.post);
...
export default SimlipalSpecies;